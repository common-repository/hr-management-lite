    ...
    public static function add_requests() {
        ...
        if ( isset( $_POST['name'] ) && isset( $_POST['start'] ) && isset( $_POST['desc'] ) && isset( $_POST['to'] ) ) {
            ...
            if ( update_option( 'ehrm_requests_data', $requests ) ) {
                ...
                if ( ! empty( $all_requests ) ) {
                    ...
                    foreach ( $all_requests as $key => $request ) {
                        ...
                        if ( $request['name'] == $name ) {
                            HRMLiteHelperClass::send_new_leave_mails( $key );
                        }
                        ...
                        $html .= '<tr>
				                	<td>'.esc_html( $sno ).'.</td>
				                  	...
				                  	<td>'.esc_html( date( HRMLiteHelperClass::get_date_format(), strtotime( $request['date'] ) ) ).'</td>
				                  	<td>'.esc_html( "From ".date( HRMLiteHelperClass::get_date_format(), strtotime( $request['start'] ) )." to ".date( HRMLiteHelperClass::get_date_format(), strtotime( $request['to'] ) ) ).'</td>
                ...
                wp_send_json( array( 'html' => wp_kses_post( $html ), 'message' => __( 'Your Request added.!', 'hr-management-lite' ) ) );
            } else {
                wp_send_json( __( 'Request not added', 'hr-management-lite' ) );
            }
        } else {
    ...
    public static function update_requests() {
        ...
        if ( isset( $_POST['name'] ) && isset( $_POST['start'] ) && isset( $_POST['desc'] ) && isset( $_POST['to'] ) ) {
            ...
            if ( $role == 'administrator'  ) {
                ...
                if ( $status == 'Approved' ) {
                    HRMLiteHelperClass::send_approved_leave_mails( $key );
                } elseif ( $status == 'Canceled' ) {
                    HRMLiteHelperClass::send_rejected_leave_mails( $key );
                }
    ...
    public static function delete_requests() {
        ...
        if ( isset( $_POST['request_key'] ) ) {
            ...
            if ( update_option( 'ehrm_requests_data', $requests ) ) {
                ...
                wp_send_json( __( 'Request not deleted', 'hr-management-lite' ) );
            }
        } else {
            wp_send_json( __( 'Something went wrong.!', 'hr-management-lite' ) );
        }
